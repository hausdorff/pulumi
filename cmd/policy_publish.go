	...
	var cmd = &cobra.Command{
		...
		Run: cmdutil.RunFunc(func(cmd *cobra.Command, args []string) error {
			...
			res := policyPack.Publish(commandContext(), backend.PublishOperation{
				Proj: proj, Root: root, PlugCtx: plugctx, Scopes: cancellationScopes})
...
func requirePolicyPack(policyPack string) (backend.PolicyPack, error) {
	...
	policy, err := b.GetPolicyPack(commandContext(), policyPackRef, cmdutil.Diag())
	if err != nil {
		return nil, err
	}
	if policy != nil {
	...
	return nil, fmt.Errorf("Could not find PolicyPack %q", policyPackRef.String())
}
